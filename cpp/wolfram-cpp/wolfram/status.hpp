...
inline void require(wf_status s) {
    if (s != WF_OK) {
        throw std::system_error(
            std::error_code(static_cast<int>(s), category()));
    }
}
...
inline std::string wolfram::status_category::message(int ev) const {
    switch (ev) {
        ...
        case WF_ERR_DID_RESOLVE:
            return "did resolve error";
        case WF_ERR_DID_DOCUMENT_NOT_FOUND:
            return "did document not found";
        case WF_ERR_HANDLE_RESOLVE:
            return "handle resolve error";
        case WF_ERR_HANDLE_DOCUMENT_NOT_FOUND:
            return "handle document not found";
        case WF_ERR_HANDLE_TTL_EXPIRED:
            return "handle ttl expired";
        case WF_ERR_HANDLE_CACHE_KEY:
            return "handle cache key error";
        case WF_ERR_CRYPTO:
            return "crypto error";
        case WF_ERR_VALIDATION:
        ...
        case WF_ERR_UNSUPPORTED:
            return "unsupported";
        case WF_ERR_PERMISSION:
            return "permission denied";
        case WF_ERR_RATE_LIMIT:
            return "rate limited";
        case WF_ERR_DUPLICATE:
            return "duplicate";
...
namespace std {
template <> struct is_error_code_enum<wf_status> : true_type {};
} // namespace std
