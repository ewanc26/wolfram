...
6. **Pure C runtime**: the SDK and generated clients are C23. Development-time code generation and tests use C++ programs (`tools/*.cpp`, built as CMake targets); they must never become a runtime dependency. C++ is permitted for performance-critical components and third-party library integrations where C proves insufficient. All C++ code must be wrapped with `extern "C"` to maintain C23 compatibility. Always use `extern "C"` for any wrapper so the rest of the SDK can consume it without C++ headers or types. Where a C library equivalent exists, prefer the C one.
