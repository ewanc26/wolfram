...
6. **Pure C runtime**: the SDK and generated clients are C11. Python is permitted only for optional development-time code generation and tests; it must never become a runtime dependency. C++ is permitted only when wrapping a third-party library that exposes no C ABI; any such wrapper must use `extern "C"` so the rest of the SDK never depends on C++ headers or types. Where a C library equivalent is available, prefer the C one.
