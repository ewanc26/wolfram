...
- `ozone`: Ozone moderation-service / labeler helper — verify and emit labels, build service auth headers. Full typed wrapper coverage across all tools.ozone.* namespaces: moderation (queryStatuses, getLabelDefs, emitEvent, queryEvents, getEvent, getReporterStats, getSubjects, getSuggestions, getAccountTimeline, getRecords, getRepo, getRepos, searchRepos, cancelScheduledActions, scheduleAction, listScheduledActions), queue (assignModerator, createQueue, deleteQueue, getAssignments, listQueues, routeReports, unassignModerator, updateQueue), report (14 endpoints), team (add/delete/list/update member), verification (grant/list/revoke), signature (findCorrelation, findRelatedAccounts, searchAccounts), setting (listOptions, removeOptions, upsertOption), hosting (getAccountHistory), server (getConfig), safelink (addRule, queryEvents, queryRules, removeRule, updateRule), and communication/set value wrappers. Tested.
...
- `validate`: `wf_validate_value` and `wf_validate_record` for runtime object/record validation against lexicon schemas. Supports `object`, `record`, `query` (parameters extraction), `procedure` (input.schema extraction), `params`, `blob`, `ref`, `array`, `string` (format validation), `integer`, `boolean`, `unknown`, and `union` types. Tested.
- `json`: generic (non-Lexicon) JSON — `wf_json_canonicalize` round-trip and `wf_json_validate` (a minimal JSON-Schema subset: type/required/properties/items). Tested.
...
- `server_xrpc`: Optional `libmicrohttpd`-backed XRPC server module (`WOLFRAM_BUILD_SERVER`). Route registration (query/procedure), auth middleware, query parameter parsing, POST body accumulation, CORS headers, integrated token-bucket rate limiter (`wf_rate_limiter`), and readback of bound port. Tested with offline round-trip, rate limiter unit, rate limiter refill, and server rate limit integration tests against the `wf_xrpc_client` transport.

## Next planned work

- Continue evaluating upstream C libraries for server-side infrastructure (event loop, config parsing, rate limiting).
