...
static void print_subject_status(
    ...
    printf("status %zu: id=%" PRId64 " review_state=%s subject_repo_handle=%s "
           ...
           idx, st->id,
           st->review_state ? st->review_state : "?",
...
static void print_team_member(
    const wf_lex_tools_ozone_team_defs_member *m, size_t idx) {
...
static void print_template(
    const wf_lex_tools_ozone_communication_defs_template_view *t,
    size_t idx) {
...
static void print_option(
    const wf_lex_tools_ozone_setting_defs_option *o, size_t idx) {
...
static void print_account(
    const wf_lex_com_atproto_admin_defs_account_view *a, size_t idx) {
...
static void print_report(
    const wf_lex_tools_ozone_report_defs_report_view *r, size_t idx) {
...
static void print_queue(
    const wf_lex_tools_ozone_queue_defs_queue_view *q, size_t idx) {
...
int cmd_ozone(int argc, char **argv) {
    ...
    /* moderation query-statuses <service> <handle> <password> [--limit N] [--cursor C] */
    if (strcmp(group, "moderation") == 0 &&
        ...
        wf_status s =
            wf_ozone_moderation_queryStatuses(agent, &params, &out);
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: queryStatuses failed (status %d)\n", (int)s);
        ...
        if (out->subject_statuses.count == 0)
            printf("(no subject statuses)\n");
    ...
    /* moderation get-suggestions <service> <handle> <password> [--ignore-subjects uri]... [--limit N] */
    if (strcmp(group, "moderation") == 0 &&
        ...
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: getSuggestions failed (status %d)\n", (int)s);
            wf_agent_free(agent);
            return 1;
        }
        if (out_json)
    ...
    /* moderation get-label-definitions <service> <handle> <password> [--uri uri]... */
    if (strcmp(group, "moderation") == 0 &&
        ...
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: getLabelDefinitions failed (status %d)\n",
                    (int)s);
            wf_agent_free(agent);
            return 1;
        }
    ...
    if (strcmp(group, "team") == 0 && strcmp(sub, "list-members") == 0) {
        ...
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: listMembers failed (status %d)\n", (int)s);
    ...
    if (strcmp(group, "server") == 0 && strcmp(sub, "get-config") == 0) {
        if (argc < 6) {
            fprintf(stderr,
                    "error: usage: wolfram ozone server get-config "
        ...
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: getConfig failed (status %d)\n", (int)s);
        ...
        if (out->has_pds && out->pds && out->pds->has_url)
            printf("pds: %s\n", out->pds->url);
        if (out->has_blob_divert && out->blob_divert && out->blob_divert->has_url)
    ...
    if (strcmp(group, "communication") == 0 &&
        ...
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: listTemplates failed (status %d)\n", (int)s);
    ...
    /* setting list-options <service> <handle> <password> [--key key] [--scope scope] [--limit N] */
    if (strcmp(group, "setting") == 0 &&
        strcmp(sub, "list-options") == 0) {
        const char *keys[256];
        size_t key_count = 0;
        ...
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: listOptions failed (status %d)\n", (int)s);
        ...
        if (out->options.count == 0)
            printf("(no settings options)\n");
    ...
    if (strcmp(group, "signature") == 0 &&
        ...
        const char *service = argv[4];
        const char *handle = argv[5];
        ...
        wf_status s =
            wf_ozone_signature_searchAccounts(agent, &params, &out);
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: searchAccounts failed (status %d)\n", (int)s);
    ...
    /* report query-reports <service> <handle> <password> [--limit N] [--cursor C] */
    if (strcmp(group, "report") == 0 &&
        strcmp(sub, "query-reports") == 0) {
        const char *cursor = NULL;
        int limit = 50;
        const char *pos[3];
        int pi = 0;
        ...
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: queryReports failed (status %d)\n", (int)s);
    ...
    if (strcmp(group, "queue") == 0 &&
        strcmp(sub, "list-queues") == 0) {
        const char *cursor = NULL;
        int limit = 50;
        const char *pos[3];
        int pi = 0;
        ...
        if (s != WF_OK) {
            fprintf(stderr,
                    "error: listQueues failed (status %d)\n", (int)s);
