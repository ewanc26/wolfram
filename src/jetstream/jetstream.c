...
static int wf_url_append_encoded(struct wf_url_buffer *buf, const char *text) {
    ...
    for (const unsigned char *p = (const unsigned char *)text; *p; ++p) {
        if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') ||
            ...
            *p == '~') {
            char one[2] = {(char)*p, 0};
            if (!wf_url_append(buf, one)) return 0;
        } else {
...
static int wf_url_param(struct wf_url_buffer *buf, int *first, const char *name,
                        const char *value) {
    if (!wf_url_append(buf, *first ? "?" : "&") || !wf_url_append(buf, name) ||
        !wf_url_append(buf, "=") || !wf_url_append_encoded(buf, value))
        return 0;
    *first = 0;
    return 1;
}
...
static wf_jetstream_event_kind wf_jetstream_kind_from_string(const char *kind) {
    ...
    if (strcmp(kind, "account_delete") == 0)
        return WF_JETSTREAM_EVENT_ACCOUNT_DELETE;
...
static int wf_jetstream_copy_options(wf_jetstream *stream,
    ...
    if (options->wanted_collections_count) {
        ...
        stream->options.wanted_collections =
            (const char *const *)stream->collections;
    }
...
static int
wf_jetstream_update_valid(const wf_jetstream_options_update *update) {
    if (!update || update->wanted_collections_count > 100 ||
        ...
        (update->wanted_dids_count && !update->wanted_dids))
        return 0;
    for (size_t i = 0; i < update->wanted_collections_count; ++i)
        if (!update->wanted_collections[i] || !update->wanted_collections[i][0])
            return 0;
...
wf_jetstream_options_update_json(const wf_jetstream_options_update *update,
    ...
    *out_json_len = 0;
    ...
    if (!root || !payload || !collections || !dids ||
        !cJSON_AddStringToObject(root, "type", "options_update"))
        goto alloc;
    cJSON_AddItemToObject(root, "payload", payload);
    payload = NULL;
    cJSON *body = cJSON_GetObjectItemCaseSensitive(root, "payload");
    cJSON_AddItemToObject(body, "wantedCollections", collections);
    collections = NULL;
    cJSON_AddItemToObject(body, "wantedDids", dids);
    dids = NULL;
    cJSON *collection_array =
        cJSON_GetObjectItemCaseSensitive(body, "wantedCollections");
...
wf_jetstream_update_options(wf_jetstream *stream,
    ...
    size_t json_len = 0;
    wf_status status =
        wf_jetstream_options_update_json(update, &json, &json_len);
    if (status != WF_OK) return status;
    ...
    if (!stream->pending_update_json) {
        ...
        free(json);
    status = wf_websocket_send_text(stream->socket, stream->pending_update_json,
                                    stream->pending_update_json_len);
    if (status != WF_ERR_WOULD_BLOCK) {
        free(stream->pending_update_json);
        stream->pending_update_json = NULL;
...
wf_status wf_jetstream_event_parse_zstd(const void *compressed,
    ...
    if (!compressed || !compressed_len || !dictionary || !dictionary_len ||
        !out)
        return WF_ERR_INVALID_ARG;
#ifdef HAVE_LIBZSTD
    ...
    size_t result =
        ZSTD_DCtx_loadDictionary(context, dictionary, dictionary_len);
    ...
    json[length] = '\0';
    wf_status status =
        wf_jetstream_event_parse((const char *)json, length, out);
    free(json);
    return status;
#else
    (void)compressed;
...
wf_status wf_jetstream_event_parse(const char *json, size_t json_len,
    ...
    cJSON *time_us =
        root ? cJSON_GetObjectItemCaseSensitive(root, "time_us") : NULL;
    ...
    int is_info =
        cJSON_IsString(kind) && strcmp(kind->valuestring, "info") == 0;
    ...
    if (!stream->socket) {
        ...
        if (reconnect != WF_OK) {
            uint32_t maximum = stream->options.reconnect_max_delay_ms
                                   ? stream->options.reconnect_max_delay_ms
                                   : 30000;
            ...
            if (stream->retry_delay_ms < maximum) {
                ...
                stream->retry_delay_ms =
                    doubled > maximum ? maximum : (uint32_t)doubled;
            }
    ...
    for (;;) {
        ...
        if (status == WF_ERR_WOULD_BLOCK) {
            ...
            uint32_t ping_interval = stream->options.ping_interval_ms
                                         ? stream->options.ping_interval_ms
                                         : 30000;
        ...
        if (message.type == WF_WEBSOCKET_TEXT && !stream->options.compress)
            ...
            status = wf_jetstream_event_parse_zstd(
                message.data, message.len, stream->dictionary,
                stream->options.zstd_dictionary_len, out);
        else
            status = WF_ERR_PARSE;
        ...
        if (out->kind == WF_JETSTREAM_EVENT_INFO) {
            ...
            cJSON *name =
                root ? cJSON_GetObjectItemCaseSensitive(root, "name") : NULL;
...
uint32_t wf_jetstream_reconnect_after_ms(const wf_jetstream *stream) {
    ...
    uint64_t now = wf_now_ms();
    uint64_t remaining =
        stream->retry_at_ms > now ? stream->retry_at_ms - now : 0;
...
wf_jetstream_commit_op_from_string(const char *op) {
...
static wf_status wf_jetstream_commit_parse(cJSON *raw,
                                           wf_jetstream_commit *out) {
...
static wf_status wf_jetstream_sync_parse(cJSON *raw,
...
wf_status wf_jetstream_event_parse_typed(const char *json, size_t json_len,
    ...
    if (!cJSON_IsObject(root) || !cJSON_IsString(kind) ||
        ...
        (!is_info && !cJSON_IsNumber(seq)) || !time_ok) {
        cJSON_Delete(root);
        return WF_ERR_PARSE;
    }
    out->kind = wf_jetstream_kind_from_string(kind->valuestring);
    ...
    switch (out->kind) {
        ...
        case WF_JETSTREAM_EVENT_INFO: {
            cJSON *payload =
                cJSON_GetObjectItemCaseSensitive(root, kind->valuestring);
    ...
    cJSON_Delete(root);
    if (status != WF_OK) {
        wf_jetstream_event_typed_free(out);
        return status;
    }
    return WF_OK;
}
