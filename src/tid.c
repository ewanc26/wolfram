/*
 * tid.c — TID (AT Protocol record key) generation clock.
 *
 * See include/wolfram/tid.h for the public API contract. The encoding is the
 * atproto "s32" (sortable base32) variant and the 64-bit value layout matches
 * the reference atproto implementation exactly, so produced TIDs are
 * interoperable and validate via wf_syntax_tid_is_valid().
 */
...
/* s32 index of a character, or -1 if out of alphabet. */
...
static pthread_mutex_t wf_tid_lock = PTHREAD_MUTEX_INITIALIZER;
...
wf_status wf_tid_now(char out[15]) {
    ...
    /* Never emit a TID that sorts <= the previous one, even if the wall
     * clock drifts backwards. */
    if (now <= wf_tid_last_ts) now = wf_tid_last_ts + 1;
    wf_tid_last_ts = now;

    uint64_t value = (now << 10) | ((uint64_t)(wf_tid_clock & 0x3FF));
    pthread_mutex_unlock(&wf_tid_lock);
