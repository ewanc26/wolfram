...
static wf_status repo_leaf_add(repo_leaf_list *list, const unsigned char *key,
                               size_t key_len, const wf_cid *cid) {
    if (key_len == 0 || !cid || cid->len != 36) return WF_ERR_PARSE;
    if (list->count > 0) {
        repo_leaf *previous = &list->items[list->count - 1];
        if (repo_key_cmp(previous->key, previous->key_len, key, key_len) >= 0)
            return WF_ERR_PARSE;
    }
    repo_leaf *items = realloc(list->items, (list->count + 1) * sizeof(*items));
    if (!items) return WF_ERR_ALLOC;
    list->items = items;
...
static wf_status repo_collect_mst(const wf_car *car, const wf_cid *root,
    ...
    for (size_t i = 0; status == WF_OK && i < node.count; i++) {
        ...
        status = repo_leaf_add(leaves, node.entries[i].key,
                               node.entries[i].key_len, &node.entries[i].value);
        if (status == WF_OK && node.entries[i].subtree.len)
            status = repo_collect_mst(car, &node.entries[i].subtree, leaves,
                                      nodes, depth + 1);
    }
...
static wf_status repo_composite_car(const wf_car *base, const wf_car *update,
    ...
    for (size_t pass = 0; pass < 2; pass++) {
        ...
        for (size_t i = 0; i < source->block_count; i++) {
            wf_car_block *present =
                wf_car_find_block(out, &source->blocks[i].cid);
            if (present) {
...
wf_status wf_repo_operations_invert(const wf_repo_operation *operations,
    ...
    *out_operations = NULL;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!source->collection || !source->collection[0] || !source->rkey ||
            !source->rkey[0] || source->cid.len != 36 ||
...
static wf_status repo_operation_from_leaf(wf_repo_operation *operation,
    ...
    if (memchr(slash + 1, '/', rkey_len) ||
        memchr(leaf->key, '\0', leaf->key_len))
        return WF_ERR_PARSE;
...
static wf_status repo_diff_operations(const repo_leaf_list *previous,
    ...
    out->operations =
        capacity ? calloc(capacity, sizeof(*out->operations)) : NULL;
    ...
    while (old_index < previous->count || new_index < current->count) {
        int comparison;
        if (old_index == previous->count)
            comparison = 1;
        else if (new_index == current->count)
            comparison = -1;
        else
            comparison = repo_key_cmp(previous->items[old_index].key,
        ...
        if (comparison < 0) {
            ...
            status =
                repo_operation_from_leaf(operation, WF_REPO_UPDATE, new_leaf);
...
static wf_status repo_diff_add_removed(wf_repo_diff *diff, const wf_cid *cid) {
    repo_cid_list view = {diff->removed_cids, diff->removed_count};
    if (repo_cid_list_has(&view, cid)) return WF_OK;
    wf_cid *items =
        realloc(diff->removed_cids, (diff->removed_count + 1) * sizeof(*items));
    if (!items) return WF_ERR_ALLOC;
...
static wf_status commit_unsigned_bytes(const wf_commit *commit,
    ...
    map->map.pairs[3].value =
        commit->has_prev ? cbor_cid(&commit->prev) : cbor_null();
...
static wf_status verify_mst_links(const wf_car *car, const wf_cid *cid,
    ...
    wf_mst_node node;
    wf_status status =
        wf_mst_node_parse(block->data, block->data_len, cid, &node);
    if (status != WF_OK) return status;
    if (node.left.len) {
...
wf_status wf_repo_diff_verify(const wf_car *base, const wf_cid *base_commit,
    ...
    if (!base || !base_commit || base_commit->len != 36 || !update ||
        !options || !out || !options->expected_did || !options->signing_key)
        return WF_ERR_INVALID_ARG;
    memset(out, 0, sizeof(*out));
    ...
    for (size_t i = 0; i < out->operation_count; i++) {
        ...
        wf_car_block *leaf =
            wf_car_find_block((wf_car *)update, &operation->cid);
        if (!leaf) {
            status = WF_ERR_NOT_FOUND;
            goto cleanup_diff;
        }
...
wf_status wf_repo_diff_apply(wf_car *repo, const wf_repo_diff *diff) {
    ...
    status =
        wf_commit_parse(commit_block->data, commit_block->data_len, &parsed);
