...
wf_cid_hasher *wf_cid_hasher_new(void) {
    wf_cid_hasher *hasher = calloc(1, sizeof(*hasher));
    if (!hasher) return NULL;
    hasher->ctx = EVP_MD_CTX_new();
    if (!hasher->ctx ||
        EVP_DigestInit_ex(hasher->ctx, EVP_sha256(), NULL) != 1) {
...
wf_status wf_cid_hasher_finish_raw(wf_cid_hasher *hasher, wf_cid *out) {
    if (!hasher || !hasher->ctx || hasher->finished || !out)
        return WF_ERR_INVALID_ARG;
