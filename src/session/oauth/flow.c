...
wf_status wf_oauth_authorization_url_create(const char *endpoint,
    ...
    if (!endpoint || !client_id || !request_uri || !out)
    ...
    *out = NULL;
    ...
    if (!escaped_client || !escaped_request) {
        ...
        curl_free(escaped_request);
        curl_easy_cleanup(curl);
        return WF_ERR_ALLOC;
    }
    separator = strchr(endpoint, '?') ? "&" : "?";
    needed = strlen(endpoint) + 1 + strlen("client_id=") +
             strlen(escaped_client) + strlen("&request_uri=") +
             strlen(escaped_request) + 1;
    url = malloc(needed);
    if (url)
        snprintf(url, needed, "%s%sclient_id=%s&request_uri=%s", endpoint,
                 separator, escaped_client, escaped_request);
    curl_free(escaped_client);
    curl_free(escaped_request);
    curl_easy_cleanup(curl);
...
wf_status wf_oauth_verify_token_subject(wf_xrpc_client *transport,
    ...
    *audience_out = NULL;
    ...
    if (status == WF_OK &&
        ...
        status = WF_ERR_PARSE;
    if (status == WF_OK)
        status = wf_oauth_discover(transport, document.pds_endpoint, &resource,
                                   &discovered);
    if (status == WF_ERR_INVALID_ARG) status = WF_ERR_PARSE;
...
static void wf_oauth_revoke_issued_token(
    ...
    value = prefer_refresh && token->refresh_token ? token->refresh_token
                                                   : token->access_token;
    if (value)
        (void)wf_oauth_revoke(transport, server->revocation_endpoint, dpop_key,
                              client_auth, value);
}
...
wf_status wf_oauth_authorization_begin(
    ...
    if (!scope || !wf_oauth_scope_has(scope, "atproto"))
        return WF_ERR_INVALID_ARG;
...
wf_status wf_oauth_authorization_complete(
    ...
    memset(out, 0, sizeof(*out));
    status = wf_oauth_authorization_state_parse(state_json, state_json_len, now,
                                                &pending);
    ...
    if (status != WF_OK) goto done;
    status = wf_oauth_verify_token_subject(transport, token.sub, pending.issuer,
                                           &audience);
    if (status != WF_OK) {
    ...
    if (status == WF_OK)
        status =
            wf_oauth_session_state_serialize(&out->session, &out->session_json);
    if (status != WF_OK) {
        wf_oauth_revoke_issued_token(transport, server, pending.dpop_key,
                                     client_auth, &token, 1);
