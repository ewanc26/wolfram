...
wf_status wf_oauth_authorization_state_create(
    ...
    if (!issuer || !pkce || !dpop_key || !out || now <= 0 || ttl_seconds <= 0 ||
        now > INT64_MAX - ttl_seconds || !wf_oauth_url_valid(issuer, 1, 1, 0))
        return WF_ERR_INVALID_ARG;
    memset(out, 0, sizeof(*out));
    if (wf_oauth_pkce_from_verifier(pkce->verifier, &(wf_oauth_pkce){0}) !=
        WF_OK) {
        return WF_ERR_INVALID_ARG;
    }
    ...
    if (client_auth_key_id)
        out->client_auth_key_id = wf_oauth_strdup(client_auth_key_id);
    status = wf_oauth_dpop_key_export(dpop_key, private_key);
    if (status == WF_OK)
        status = wf_oauth_dpop_key_import(private_key, &out->dpop_key);
    OPENSSL_cleanse(private_key, sizeof(private_key));
    if (!out->issuer || !out->code_verifier || (app_state && !out->app_state) ||
        (client_auth_key_id &&
         (!*client_auth_key_id || !out->client_auth_key_id)) ||
        status != WF_OK) {
...
static wf_status wf_oauth_private_jwk(const wf_oauth_dpop_key *key,
                                      cJSON **out) {
    cJSON *jwk = NULL;
    ...
    if (status == WF_OK)
        status = wf_oauth_base64url(private_key, sizeof(private_key), &d);
    OPENSSL_cleanse(private_key, sizeof(private_key));
    if (status == WF_OK && !cJSON_AddStringToObject(jwk, "d", d))
        status = WF_ERR_ALLOC;
    free(x);
    free(y);
    free(d);
    ...
    *json_out = NULL;
    ...
    if (status == WF_OK &&
        (!cJSON_AddStringToObject(root, "iss", state->issuer) ||
         ...
         !cJSON_AddNumberToObject(root, "expiresAt",
                                  (double)state->expires_at)))
        status = WF_ERR_ALLOC;
    if (status == WF_OK &&
        (!cJSON_AddStringToObject(auth, "method",
                                  state->client_auth_key_id ? "private_key_jwt"
                                                            : "none") ||
         (state->client_auth_key_id &&
          !cJSON_AddStringToObject(auth, "kid", state->client_auth_key_id))))
        status = WF_ERR_ALLOC;
    if (status == WF_OK) {
        if (!cJSON_AddItemToObject(root, "authMethod", auth))
            status = WF_ERR_ALLOC;
        else
            auth = NULL;
    }
    if (status == WF_OK) {
        if (!cJSON_AddItemToObject(root, "dpopJwk", jwk))
            status = WF_ERR_ALLOC;
        else
            jwk = NULL;
    }
    if (status == WF_OK && state->app_state &&
        !cJSON_AddStringToObject(root, "appState", state->app_state))
        status = WF_ERR_ALLOC;
    if (status == WF_OK) {
...
static wf_status wf_oauth_base64url_32_decode(const char *encoded,
    ...
    for (i = 0; i < 43; i++) {
        if (padded[i] == '-')
            ...
            padded[i] = '/';
        else if (!isalnum((unsigned char)padded[i]) && padded[i] != '+' &&
                 padded[i] != '/')
            return WF_ERR_PARSE;
    }
...
#undef REQUIRED_JWK_STRING
    ...
    OPENSSL_cleanse(private_key, sizeof(private_key));
    if (status == WF_OK)
        status = wf_oauth_dpop_jwk(key, &expected, &expected_x, &expected_y);
    if (status == WF_OK &&
...
wf_status
wf_oauth_authorization_state_parse(const char *json, size_t len, int64_t now,
                                   wf_oauth_authorization_state *out) {
    cJSON *root = NULL;
    const cJSON *jwk, *item;
    char *issuer = NULL, *verifier = NULL, *app_state = NULL,
         *auth_key_id = NULL;
    const char *x, *y, *d;
    unsigned char private_key[32];
    wf_oauth_dpop_key *key = NULL;
    ...
    if (status == WF_OK)
        status = wf_oauth_json_string(root, "verifier", 1, &verifier);
    if (status == WF_OK)
        status = wf_oauth_json_string(root, "appState", 0, &app_state);
    if (status == WF_OK)
        status =
            wf_oauth_positive_integer(root, "expiresAt", 1, &expires_at, NULL);
    {
        const cJSON *auth =
            cJSON_GetObjectItemCaseSensitive(root, "authMethod");
        const cJSON *method =
            auth ? cJSON_GetObjectItemCaseSensitive(auth, "method") : NULL;
        if (status == WF_OK &&
            (!cJSON_IsObject(auth) || !cJSON_IsString(method) ||
        ...
        if (status == WF_OK && strcmp(method->valuestring, "none") == 0 &&
            cJSON_GetObjectItemCaseSensitive(auth, "kid"))
            status = WF_ERR_PARSE;
    }
    ...
    if (status == WF_OK && (!cJSON_IsObject(jwk) || !cJSON_IsString(item) ||
                            strcmp(item->valuestring, "EC") != 0))
        status = WF_ERR_PARSE;
#define JWK_STRING(name, target)                                               \
    do {                                                                       \
        item = jwk ? cJSON_GetObjectItemCaseSensitive(jwk, name) : NULL;       \
        target = cJSON_IsString(item) ? item->valuestring : NULL;              \
        if (status == WF_OK && !target) status = WF_ERR_PARSE;                 \
    } while (0)
    JWK_STRING("crv", x);
    if (status == WF_OK && strcmp(x, "P-256") != 0) status = WF_ERR_PARSE;
    JWK_STRING("x", x);
    JWK_STRING("y", y);
    JWK_STRING("d", d);
#undef JWK_STRING
    if (status == WF_OK &&
        (!wf_oauth_url_valid(issuer, 1, 1, 0) || expires_at <= now))
        status = WF_ERR_PARSE;
    if (status == WF_OK &&
        wf_oauth_pkce_from_verifier(verifier, &(wf_oauth_pkce){0}) != WF_OK)
        status = WF_ERR_PARSE;
    ...
    if (status == WF_OK)
        status =
            wf_oauth_dpop_jwk(key, &expected_jwk, &expected_x, &expected_y);
    if (status == WF_OK &&
        (strcmp(x, expected_x) != 0 || strcmp(y, expected_y) != 0))
        status = WF_ERR_PARSE;
    if (status == WF_OK) {
        ...
        app_state = NULL;
        out->client_auth_key_id = auth_key_id;
        auth_key_id = NULL;
...
wf_status wf_oauth_session_state_create(
    ...
    const wf_oauth_token_response *token, const char *client_auth_key_id,
    int64_t now, wf_oauth_session_state *out) {
    unsigned char private_key[32];
    wf_status status;
    ...
    if (token->refresh_token)
        out->refresh_token = wf_oauth_strdup(token->refresh_token);
    if (client_auth_key_id)
        out->client_auth_key_id = wf_oauth_strdup(client_auth_key_id);
    status = wf_oauth_dpop_key_export(dpop_key, private_key);
    if (status == WF_OK)
...
wf_oauth_session_state_serialize(const wf_oauth_session_state *session,
    ...
    *json_out = NULL;
    ...
    auth = cJSON_CreateObject();
    status = root && tokens && auth
                 ? wf_oauth_private_jwk(session->dpop_key, &jwk)
                 : WF_ERR_ALLOC;
    if (status == WF_OK &&
        (!cJSON_AddStringToObject(tokens, "iss", session->issuer) ||
         ...
         !cJSON_AddStringToObject(tokens, "access_token",
                                  session->access_token) ||
         !cJSON_AddStringToObject(tokens, "token_type", "DPoP")))
        status = WF_ERR_ALLOC;
    if (status == WF_OK &&
        (!cJSON_AddStringToObject(
             auth, "method",
             session->client_auth_key_id ? "private_key_jwt" : "none") ||
    ...
    if (status == WF_OK && session->refresh_token &&
        !cJSON_AddStringToObject(tokens, "refresh_token",
                                 session->refresh_token))
        status = WF_ERR_ALLOC;
    if (status == WF_OK && session->expires_at > 0 &&
        !cJSON_AddNumberToObject(tokens, "expires_at",
                                 (double)session->expires_at))
        status = WF_ERR_ALLOC;
    if (status == WF_OK) {
        if (!cJSON_AddItemToObject(root, "tokenSet", tokens))
            status = WF_ERR_ALLOC;
        else
            tokens = NULL;
    }
...
wf_status wf_oauth_session_state_parse(const char *json, size_t len,
    ...
    if (!json || !len || !expected_subject || !out ||
        strncmp(expected_subject, "did:", 4) != 0)
        return WF_ERR_INVALID_ARG;
    memset(out, 0, sizeof(*out));
    root = cJSON_ParseWithLength(json, len);
    if (!root || !cJSON_IsObject(root)) {
    ...
    if (!cJSON_IsObject(auth) || !cJSON_IsString(method) ||
        (strcmp(method->valuestring, "none") != 0 &&
         strcmp(method->valuestring, "private_key_jwt") != 0) ||
        !cJSON_IsObject(tokens)) {
        status = WF_ERR_PARSE;
        goto done;
    }
    status = wf_oauth_json_string(tokens, "iss", 1, &out->issuer);
    if (status == WF_OK)
        status = wf_oauth_json_string(tokens, "sub", 1, &out->subject);
    if (status == WF_OK)
        status = wf_oauth_json_string(tokens, "aud", 1, &out->audience);
    if (status == WF_OK)
        status = wf_oauth_json_string(tokens, "scope", 1, &out->scope);
    if (status == WF_OK)
        status =
            wf_oauth_json_string(tokens, "access_token", 1, &out->access_token);
    if (status == WF_OK)
        status = wf_oauth_json_string(tokens, "refresh_token", 0,
                                      &out->refresh_token);
    ...
    if (status == WF_OK && strcmp(method->valuestring, "none") == 0 &&
        ...
        status = WF_ERR_PARSE;
    if (status == WF_OK)
        status = wf_oauth_positive_integer(tokens, "expires_at", 0,
                                           &out->expires_at, NULL);
    type = cJSON_GetObjectItemCaseSensitive(tokens, "token_type");
    if (status == WF_OK &&
        (!cJSON_IsString(type) || strcmp(type->valuestring, "DPoP") != 0))
        status = WF_ERR_PARSE;
    if (status == WF_OK && (!wf_oauth_url_valid(out->issuer, 1, 1, 0) ||
                            !wf_oauth_url_valid(out->audience, 1, 1, 0) ||
                            strcmp(out->subject, expected_subject) != 0))
        status = WF_ERR_PARSE;
    if (status == WF_OK)
        status = wf_oauth_private_jwk_parse(jwk, &out->dpop_key);
done:
    cJSON_Delete(root);
