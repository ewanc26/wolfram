...
static char *wf_oauth_origin(const char *url) {
    ...
    if (curl_url_get(parsed, CURLUPART_PORT, &port, 0) != CURLUE_OK)
        port = NULL;
...
static wf_status wf_oauth_metadata_get(wf_xrpc_client *transport,
                                       const char *url, const char *expected,
                                       wf_oauth_parse_metadata_fn parse,
                                       void *out) {
    wf_response response = {0};
    wf_status status;
    if (!transport || !url || !expected || !parse || !out)
        return WF_ERR_INVALID_ARG;
    status = wf_http_get(transport, url, &response);
    if (status == WF_OK && response.status != 200) status = WF_ERR_HTTP;
    if (status == WF_OK)
        status = parse(response.body, response.body_len, expected, out);
...
wf_status wf_oauth_resource_metadata_parse(const char *json, size_t json_len,
    ...
    if (status == WF_OK)
        status = wf_oauth_json_array(root, "authorization_servers", 1,
                                     &out->authorization_servers);
    if (status == WF_OK)
        status = wf_oauth_json_array(root, "scopes_supported", 0,
                                     &out->scopes_supported);
    cJSON_Delete(root);
    if (status == WF_OK && !wf_oauth_url_valid(out->resource, 1, 1, 0)) {
        status = WF_ERR_PARSE;
    }
    if (status == WF_OK && expected_resource &&
        strcmp(out->resource, expected_resource) != 0) {
        status = WF_ERR_PARSE;
    }
    if (status == WF_OK && out->authorization_servers.count != 1)
        status = WF_ERR_PARSE;
    if (status == WF_OK &&
...
#define WF_OAUTH_GET_STRING(field, name, required)                             \
    do {                                                                       \
        if (status == WF_OK)                                                   \
            status = wf_oauth_json_string(root, name, required, &out->field);  \
    } while (0)
#define WF_OAUTH_GET_ARRAY(field, name)                                        \
    do {                                                                       \
        if (status == WF_OK)                                                   \
            status = wf_oauth_json_array(root, name, 1, &out->field);          \
    } while (0)
    status = WF_OK;
    ...
    WF_OAUTH_GET_ARRAY(code_challenge_methods_supported,
    ...
    if (status == WF_OK)
        status = wf_oauth_json_array(root, "protected_resources", 0,
                                     &out->protected_resources);
    if (status == WF_OK)
        status = wf_oauth_json_bool(
    ...
    if (status == WF_OK)
        status =
            wf_oauth_json_bool(root, "require_request_uri_registration",
...
#undef WF_OAUTH_GET_STRING
    cJSON_Delete(root);
    if (status == WF_OK &&
        (!wf_oauth_url_valid(out->issuer, 1, 1, 0) ||
         ...
         !wf_oauth_url_valid(out->pushed_authorization_request_endpoint, 1, 0,
                             0))) {
        status = WF_ERR_PARSE;
    }
    if (status == WF_OK && expected_issuer &&
        strcmp(out->issuer, expected_issuer) != 0) {
        status = WF_ERR_PARSE;
    }
    if (status == WF_OK &&
        (!wf_oauth_string_list_has(&out->response_types_supported, "code") ||
         !wf_oauth_string_list_has(&out->grant_types_supported,
                                   "authorization_code") ||
         !wf_oauth_string_list_has(&out->grant_types_supported,
                                   "refresh_token") ||
         !wf_oauth_string_list_has(&out->code_challenge_methods_supported,
                                   "S256") ||
         ...
         !wf_oauth_string_list_has(
             &out->token_endpoint_auth_signing_alg_values_supported, "ES256") ||
         wf_oauth_string_list_has(
             &out->token_endpoint_auth_signing_alg_values_supported, "none") ||
         !wf_oauth_string_list_has(&out->scopes_supported, "atproto") ||
         !wf_oauth_string_list_has(&out->dpop_signing_alg_values_supported,
                                   "ES256") ||
...
static wf_status wf_oauth_default_array(wf_oauth_string_list *list,
                                        const char *value) {
...
wf_status wf_oauth_client_metadata_parse(const char *json, size_t json_len,
    ...
    if (status == WF_OK)
        status =
            wf_oauth_json_string(root, "client_name", 0, &out->client_name);
    if (status == WF_OK)
        status = wf_oauth_json_string(root, "client_uri", 0, &out->client_uri);
    if (status == WF_OK)
        status = wf_oauth_json_string(root, "scope", 1, &out->scope);
    if (status == WF_OK)
        status = wf_oauth_json_string(root, "token_endpoint_auth_method", 1,
                                      &out->token_endpoint_auth_method);
    if (status == WF_OK)
        status = wf_oauth_json_string(root, "token_endpoint_auth_signing_alg",
                                      0, &out->token_endpoint_auth_signing_alg);
    if (status == WF_OK)
        status = wf_oauth_json_string(root, "jwks_uri", 0, &out->jwks_uri);
    if (status == WF_OK)
        status = wf_oauth_json_object_encoded(root, "jwks", &out->jwks_json);
    if (status == WF_OK)
        status = wf_oauth_json_string(root, "application_type", 0,
    ...
    if (status == WF_OK)
        status =
            wf_oauth_json_array(root, "redirect_uris", 1, &out->redirect_uris);
    if (status == WF_OK)
        status = wf_oauth_json_array(root, "response_types", 0,
                                     &out->response_types);
    if (status == WF_OK && out->response_types.count == 0) {
        status = wf_oauth_default_array(&out->response_types, "code");
    }
    if (status == WF_OK)
        status = wf_oauth_json_array(root, "grant_types", 0, &out->grant_types);
    if (status == WF_OK && out->grant_types.count == 0) {
        status =
            wf_oauth_default_array(&out->grant_types, "authorization_code");
    }
    if (status == WF_OK)
        status = wf_oauth_json_bool(root, "dpop_bound_access_tokens",
                                    &out->dpop_bound_access_tokens,
                                    &out->dpop_bound_access_tokens_present);
    cJSON_Delete(root);
    has_jwks = out->jwks_json != NULL;
    if (status == WF_OK &&
        (!wf_oauth_client_id_valid(out->client_id) ||
         (expected_client_id &&
         ...
         !wf_oauth_string_list_has(&out->grant_types, "authorization_code") ||
         !out->dpop_bound_access_tokens_present ||
         !out->dpop_bound_access_tokens || (out->jwks_uri && has_jwks))) {
        status = WF_ERR_PARSE;
    }
    ...
    if (status == WF_OK && out->jwks_uri &&
        !wf_oauth_url_valid(out->jwks_uri, 1, 0, 0))
        status = WF_ERR_PARSE;
    if (status == WF_OK &&
        strcmp(out->token_endpoint_auth_method, "none") == 0) {
        if (out->token_endpoint_auth_signing_alg) status = WF_ERR_PARSE;
    } else if (status == WF_OK && strcmp(out->token_endpoint_auth_method,
                                         "private_key_jwt") == 0) {
...
wf_status wf_oauth_resource_metadata_get(wf_xrpc_client *transport,
    ...
    char *url, *origin;
    wf_status status;
    if (!transport || !resource || !out ||
        !wf_oauth_url_valid(resource, 1, 1, 0)) {
        return WF_ERR_INVALID_ARG;
    }
    origin = wf_oauth_origin(resource);
    if (!origin) return WF_ERR_ALLOC;
    url = wf_oauth_well_known_url(origin,
...
wf_status wf_oauth_discover(wf_xrpc_client *transport, const char *resource,
    ...
    status = wf_oauth_server_metadata_get(
        transport, resource_out->authorization_servers.items[0], server_out);
