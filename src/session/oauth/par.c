...
wf_status wf_oauth_par_response_parse(const char *json, size_t len,
    ...
    if (s == WF_OK)
        s = wf_oauth_positive_integer(root, "expires_in", 1, &out->expires_in,
                                      NULL);
    cJSON_Delete(root);
...
wf_status wf_oauth_token_response_parse(const char *json, size_t len,
    ...
    if (s == WF_OK)
        s = wf_oauth_json_string(root, "token_type", 1, &out->token_type);
    if (s == WF_OK) s = wf_oauth_json_string(root, "sub", 1, &out->sub);
    if (s == WF_OK) s = wf_oauth_json_string(root, "scope", 1, &out->scope);
    if (s == WF_OK)
        s = wf_oauth_json_string(root, "refresh_token", 0, &out->refresh_token);
    if (s == WF_OK)
        s = wf_oauth_positive_integer(root, "expires_in", 0, &out->expires_in,
    ...
    if (s == WF_OK && !wf_oauth_scope_has(out->scope, "atproto"))
        s = WF_ERR_PARSE;
...
static wf_status wf_oauth_form_encode(const wf_xrpc_param *params, size_t count,
    ...
    *out = i == count ? malloc(len) : NULL;
    if (*out)
        for (i = 0; i < count; i++)
            off +=
                (size_t)snprintf(*out + off, len - off, "%s%s=%s", i ? "&" : "",
...
static int wf_oauth_use_nonce(const wf_response *r) {
    ...
    yes = cJSON_IsString(error) &&
          strcmp(error->valuestring, "use_dpop_nonce") == 0;
    cJSON_Delete(root);
    return yes;
}
...
static wf_status wf_oauth_post(wf_xrpc_client *transport, const char *endpoint,
                               ...
                               wf_response *out) {
    char *proof = NULL;
    wf_http_header header;
    wf_status s;
    int attempt;
    char *nonce = NULL;
    for (attempt = 0; attempt < 2; attempt++) {
        wf_oauth_dpop_proof_options opts = {"POST", endpoint, nonce,
                                            NULL,   NULL,     0};
        s = wf_oauth_dpop_proof_create(key, &opts, &proof);
        if (s != WF_OK) return s;
        header.name = "DPoP";
        header.value = proof;
        s = wf_http_post(transport, endpoint,
                         "application/x-www-form-urlencoded", form, &header, 1,
                         out);
        free(proof);
        proof = NULL;
...
static int wf_oauth_jwks_has_signing_key(const char *jwks_json,
    ...
    cJSON_ArrayForEach(key, keys) {
        ...
        if (!cJSON_IsString(item) || strcmp(item->valuestring, key_id) != 0)
            continue;
        ...
        if (item &&
            (!cJSON_IsString(item) || strcmp(item->valuestring, "ES256") != 0))
            continue;
        item = cJSON_GetObjectItemCaseSensitive(key, "use");
        if (item &&
            (!cJSON_IsString(item) || strcmp(item->valuestring, "sig") != 0))
            continue;
...
wf_status wf_oauth_client_auth_validate(const wf_oauth_server_metadata *server,
    ...
    if (strcmp(client->token_endpoint_auth_method, method) != 0 ||
        !wf_oauth_string_list_has(
            &server->token_endpoint_auth_methods_supported, method))
        return WF_ERR_PARSE;
    if (!auth->signing_key) {
    ...
    if (!auth->key_id || !*auth->key_id || !auth->authorization_server_issuer ||
        ...
        !wf_oauth_string_list_has(
            &server->token_endpoint_auth_signing_alg_values_supported,
            "ES256") ||
...
static wf_status wf_oauth_add_client_auth(wf_xrpc_param *params, size_t *count,
    ...
    *assertion_out = NULL;
    ...
    options = (wf_oauth_client_assertion_options){
        auth->client_id, auth->authorization_server_issuer, auth->key_id, NULL,
        0};
...
wf_status wf_oauth_par_with_auth(wf_xrpc_client *t, const char *endpoint,
                                 const wf_oauth_dpop_key *key,
                                 const wf_oauth_client_auth *auth,
                                 const wf_oauth_par_request *r,
                                 wf_oauth_par_response *out) {
    wf_xrpc_param p[11];
    size_t n = 0;
    char *form = NULL, *assertion = NULL;
    wf_response res = {0};
    wf_status s;
    if (!t || !endpoint || !key || !auth || !auth->client_id || !r || !out ||
        !r->client_id || strcmp(r->client_id, auth->client_id) != 0 ||
        !r->redirect_uri || !r->scope || !r->state || !r->code_challenge)
        return WF_ERR_INVALID_ARG;
#define ADD(k, v)                                                              \
    do {                                                                       \
        p[n++] = (wf_xrpc_param){k, v};                                        \
    } while (0)
    s = wf_oauth_add_client_auth(p, &n, auth, &assertion);
    ADD("redirect_uri", r->redirect_uri);
    ADD("scope", r->scope);
    ADD("state", r->state);
    ADD("code_challenge", r->code_challenge);
    ...
    if (s == WF_OK)
        s = wf_oauth_par_response_parse(res.body, res.body_len, out);
...
wf_status wf_oauth_exchange_code_with_auth(
    ...
    if (s == WF_OK) s = wf_oauth_post(t, endpoint, key, form, &res);
    if (s == WF_OK)
        s = wf_oauth_token_response_parse(res.body, res.body_len, out);
    wf_response_free(&res);
    free(assertion);
    free(form);
    return s;
...
wf_status wf_oauth_refresh_with_auth(wf_xrpc_client *t, const char *endpoint,
                                     const wf_oauth_dpop_key *key,
                                     const wf_oauth_client_auth *auth,
    ...
    if (!t || !endpoint || !key || !auth || !refresh_token || !expected_sub ||
        !out || strncmp(expected_sub, "did:", 4) != 0)
        return WF_ERR_INVALID_ARG;
...
wf_status wf_oauth_session_refresh(wf_xrpc_client *transport,
    ...
    if (status != WF_OK) return status;
    status = wf_oauth_verify_token_subject(transport, token.sub, server->issuer,
                                           &audience);
    if (status != WF_OK) {
    ...
    char *new_access_token =
        token.access_token ? wf_oauth_strdup(token.access_token) : NULL;
    char *new_refresh_token =
        token.refresh_token ? wf_oauth_strdup(token.refresh_token) : NULL;
    char *new_scope = token.scope ? wf_oauth_strdup(token.scope) : NULL;
    int64_t new_expires_at = (token.expires_in_present)
...
wf_status wf_oauth_revoke(wf_xrpc_client *transport, const char *endpoint,
    ...
    if (status == WF_OK)
        status = wf_oauth_post(transport, endpoint, dpop_key, form, &response);
