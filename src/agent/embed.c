...
cJSON *wf_embed_images_new(void) {
    ...
    if (!cJSON_AddStringToObject(embed, "$type",
                                 wf_embed_type_str(WF_EMBED_IMAGES))) {
        cJSON_Delete(embed);
        return NULL;
    }
...
wf_status wf_embed_images_add_image(cJSON *embed, const wf_uploaded_blob *blob,
                                    const char *alt) {
...
cJSON *wf_embed_video_new(const wf_uploaded_blob *blob, const char *alt) {
    ...
    if (!embed) return NULL;
    if (!cJSON_AddStringToObject(embed, "$type",
                                 wf_embed_type_str(WF_EMBED_VIDEO))) {
        cJSON_Delete(embed);
        return NULL;
    }
    ...
    if (!cJSON_AddItemToObject(embed, "video", blob_obj)) {
        cJSON_Delete(embed);
        return NULL;
    }
    if (alt && alt[0]) {
        if (!cJSON_AddStringToObject(embed, "alt", alt)) {
            cJSON_Delete(embed);
            return NULL;
        }
    }
...
cJSON *wf_embed_external_new(const char *uri, const char *thumb,
                             const char *title, const char *description) {
    if (!uri) return NULL;
    cJSON *embed = cJSON_CreateObject();
    if (!embed) return NULL;
    if (!cJSON_AddStringToObject(embed, "$type",
                                 wf_embed_type_str(WF_EMBED_EXTERNAL))) {
        cJSON_Delete(embed);
        return NULL;
    }
    if (!cJSON_AddStringToObject(embed, "uri", uri)) {
        cJSON_Delete(embed);
        return NULL;
    }
    if (thumb && thumb[0]) {
        if (!cJSON_AddStringToObject(embed, "thumb", thumb)) {
            cJSON_Delete(embed);
            return NULL;
        }
    }
    if (title && title[0]) {
        if (!cJSON_AddStringToObject(embed, "title", title)) {
            cJSON_Delete(embed);
            return NULL;
        }
    }
    if (description && description[0]) {
        if (!cJSON_AddStringToObject(embed, "description", description)) {
            cJSON_Delete(embed);
            return NULL;
        }
    }
...
cJSON *wf_embed_record_new(const char *uri, const char *cid) {
    ...
    if (!embed) return NULL;
    if (!cJSON_AddStringToObject(embed, "$type",
                                 wf_embed_type_str(WF_EMBED_RECORD))) {
        cJSON_Delete(embed);
        return NULL;
    }
    ...
    if (!cJSON_AddStringToObject(record, "uri", uri)) {
        cJSON_Delete(record);
        cJSON_Delete(embed);
        return NULL;
    }
    if (!cJSON_AddStringToObject(record, "cid", cid)) {
        cJSON_Delete(record);
        cJSON_Delete(embed);
        return NULL;
    }
    if (!cJSON_AddItemToObject(embed, "record", record)) {
        cJSON_Delete(record);
        cJSON_Delete(embed);
        return NULL;
    }
...
cJSON *wf_embed_record_with_media_new(cJSON *record_embed, cJSON *media_embed) {
    ...
    if (!cJSON_AddStringToObject(
            embed, "$type", wf_embed_type_str(WF_EMBED_RECORD_WITH_MEDIA))) {
        cJSON_Delete(embed);
        return NULL;
    }
    // Both inputs are objects; attach them and transfer ownership.
    if (!cJSON_AddItemToObject(embed, "record", record_embed)) {
        cJSON_Delete(record_embed);
        cJSON_Delete(embed);
        return NULL;
    }
    if (!cJSON_AddItemToObject(embed, "media", media_embed)) {
