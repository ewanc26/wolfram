...
int main(void) {
    ...
    size_t update_json_len = 0;
    WF_CHECK(wf_jetstream_options_update_json(&update, &update_json,
                                              &update_json_len) == WF_OK);
    WF_CHECK(update_json &&
             strcmp(update_json,
                    "{\"type\":\"options_update\",\"payload\":{"
                    "\"wantedCollections\":[\"app.bsky.feed.post\",\"app.bsky."
                    "graph.*\"],"
                    "\"wantedDids\":[\"did:plc:a/"
                    "b\"],\"maxMessageSizeBytes\":1000000}}") == 0);
    ...
    WF_CHECK(wf_jetstream_options_update_json(&update, &update_json,
                                              &update_json_len) ==
             WF_ERR_INVALID_ARG);
    update.wanted_collections_count = 0;
    update.wanted_collections = NULL;
    update.wanted_dids_count = 0;
    update.wanted_dids = NULL;
    WF_CHECK(wf_jetstream_options_update_json(&update, &update_json,
                                              &update_json_len) == WF_OK);
    WF_CHECK(strcmp(update_json,
                    "{\"type\":\"options_update\",\"payload\":{"
                    "\"wantedCollections\":[],"
                    "\"wantedDids\":[],\"maxMessageSizeBytes\":1000000}}") ==
             0);
    free(update_json);
...
#ifdef HAVE_LIBZSTD
    {
        ...
        size_t compressed_len = ZSTD_compress_usingDict(
            context, compressed, bound, json, sizeof(json) - 1, dictionary,
            sizeof(dictionary) - 1, 3);
        ...
        WF_CHECK(event.time_us == 1725516666833633LL);
        wf_jetstream_event_free(&event);
        WF_CHECK(wf_jetstream_event_parse_zstd(compressed, compressed_len,
                                               "wrong", 5,
                                               &event) == WF_ERR_PARSE);
