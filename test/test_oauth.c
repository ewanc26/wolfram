...
static void test_metadata(void) {
    ...
    WF_CHECK(
        strcmp(server.revocation_endpoint, "https://auth.example/revoke") == 0);
    WF_CHECK(server.protected_resources.count == 1);
    wf_oauth_server_metadata_free(&server);
    WF_CHECK(wf_oauth_server_metadata_parse(bad_server_json,
                                            strlen(bad_server_json), NULL,
                                            &server) == WF_ERR_PARSE);
    ...
    WF_CHECK(wf_oauth_client_metadata_parse(
                 private_client_json, strlen(private_client_json),
                 "https://app.example/oauth-client-metadata.json",
    ...
    if (auth_key) {
        ...
        WF_CHECK(wf_oauth_client_auth_validate(&server, &private_client,
                                               &auth) == WF_ERR_INVALID_ARG);
...
static void test_endpoint_responses(void) {
    ...
    static const char par_json[] = "{\"request_uri\":\"urn:ietf:params:oauth:"
                                   "request_uri:abc\",\"expires_in\":300}";
    ...
    WF_CHECK(wf_oauth_par_response_parse(par_json, strlen(par_json), &par) ==
             WF_OK);
    WF_CHECK(par.expires_in == 300);
    wf_oauth_par_response_free(&par);
    WF_CHECK(wf_oauth_par_response_parse("{\"expires_in\":0}", 16, &par) ==
             WF_ERR_PARSE);
    WF_CHECK(wf_oauth_token_response_parse(token_json, strlen(token_json),
                                           &token) == WF_OK);
    ...
    WF_CHECK(wf_oauth_token_response_parse(
                 "{\"access_token\":\"x\",\"token_type\":\"Bearer\",\"sub\":"
                 "\"did:plc:x\",\"scope\":\"atproto\"}",
                 85, &token) == WF_ERR_PARSE);
    WF_CHECK(
        wf_oauth_token_response_parse(
            "{\"access_token\":\"x\",\"token_type\":\"DPoP\",\"sub\":\"did:plc:"
            "x\",\"scope\":\"transition:generic\"}",
            strlen("{\"access_token\":\"x\",\"token_type\":\"DPoP\",\"sub\":"
                   "\"did:plc:x\",\"scope\":\"transition:generic\"}"),
            &token) == WF_ERR_PARSE);
}
...
static void test_authorization_url(void) {
    ...
    WF_CHECK(url != NULL);
    if (url) {
        WF_CHECK(strcmp(url, "https://auth.example/"
                             "authorize?client_id=https%3A%2F%2Fapp.example%"
                             "2Fclient.json&request_uri=urn%3Aietf%3Aparams%"
                             "3Aoauth%3Arequest_uri%3Aa%2Fb%2Bc") == 0);
    }
...
static void test_authorization_state(void) {
    ...
    WF_CHECK(wf_oauth_pkce_from_verifier(
                 "dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk", &pkce) ==
             WF_OK);
    ...
    if (json) {
        ...
        WF_CHECK(wf_oauth_authorization_state_parse(json, strlen(json),
                                                    1700000600,
        ...
        if (tampered) {
            ...
            if (x) x[5] = x[5] == 'A' ? 'B' : 'A';
            WF_CHECK(wf_oauth_authorization_state_parse(
                         tampered, strlen(tampered), 1700000300, &restored) ==
                     WF_ERR_PARSE);
        }
    }
    free(tampered);
...
static void test_session_refresh_args(void) {
    ...
    WF_CHECK(wf_oauth_session_refresh(transport, &server, &auth, &session,
                                      1700000000) == WF_ERR_INVALID_ARG);
}
...
static void test_session_state(void) {
    ...
    if (json) {
        ...
        if (tampered) {
            ...
            WF_CHECK(sub != NULL);
            if (sub) sub[8] = 'm';
            WF_CHECK(wf_oauth_session_state_parse(tampered, strlen(tampered),
                                                  "did:plc:alice",
                                                  &restored) == WF_ERR_PARSE);
        }
    }
...
static void test_authorization_complete_denial(void) {
    ...
    WF_CHECK(wf_oauth_dpop_key_import(private_key, &key) == WF_OK);
    WF_CHECK(wf_oauth_pkce_from_verifier("abcdefghijklmnopqrstuvwxyzABCDEFGHIJK"
                                         "LMNOPQRSTUVWXYZ0123456789-._~abc",
                                         &pkce) == WF_OK);
    WF_CHECK(wf_oauth_authorization_state_create(
                 "https://auth.example", &pkce, key, "return-to-feed", NULL,
                 1700000000, 600, &pending) == WF_OK);
    WF_CHECK(wf_oauth_authorization_state_serialize(&pending, &state_json) ==
             WF_OK);
    WF_CHECK(wf_oauth_server_metadata_parse(server_json, strlen(server_json),
                                            "https://auth.example",
                                            &server) == WF_OK);
    WF_CHECK(wf_oauth_client_metadata_parse(client_json, strlen(client_json),
                                            "https://app.example/client.json",
                                            &client) == WF_OK);
    if (transport && state_json) {
        WF_CHECK(wf_oauth_authorization_complete(
                     ...
                     "https://app.example/callback", 1700000300,
                     &result) == WF_ERR_HTTP);
        WF_CHECK(result.session_json == NULL);
        WF_CHECK(result.app_state &&
                 strcmp(result.app_state, "return-to-feed") == 0);
        ...
        WF_CHECK(wf_oauth_authorization_complete(
                     transport, &server, &client, &auth, &denied, "flow-state",
                     state_json, strlen(state_json),
                     "https://app.example/callback", 1700000600,
                     &result) == WF_ERR_PARSE);
        wf_oauth_authorization_complete_result_free(&result);
    }
    wf_oauth_client_metadata_free(&client);
...
static void test_dpop(void) {
    ...
    unsigned char *header_bytes = NULL, *payload_bytes = NULL,
                  *sig_bytes = NULL;
    ...
    WF_CHECK(
        strcmp(thumbprint, "xx0BcA-wMohw8atYDJOe6peGModklG2wRHBlXHMvl0M") == 0);
    ...
    WF_CHECK(first_dot != NULL && second_dot != NULL &&
             strchr(second_dot + 1, '.') == NULL);
    if (!first_dot || !second_dot) goto done;
    header_bytes =
        base64url_decode(jwt, (size_t)(first_dot - jwt), &header_len);
    payload_bytes = base64url_decode(
        first_dot + 1, (size_t)(second_dot - first_dot - 1), &payload_len);
    sig_bytes =
        base64url_decode(second_dot + 1, strlen(second_dot + 1), &sig_len);
    WF_CHECK(header_bytes != NULL && payload_bytes != NULL &&
             sig_bytes != NULL);
    WF_CHECK(sig_len == 64);
    if (!header_bytes || !payload_bytes || !sig_bytes || sig_len != 64)
        goto done;
    ...
    WF_CHECK(cJSON_IsString(field) &&
             strcmp(field->valuestring, "dpop+jwt") == 0);
    ...
    WF_CHECK(cJSON_IsString(field) &&
             strcmp(field->valuestring,
                    "Pxa-1wifRlPl7yG_0oJNfzqq7MelmOfonFgOFgapzFI") == 0);

    verify_key = EC_KEY_new_by_curve_name(NID_X9_62_prime256v1);
    WF_CHECK(verify_key != NULL);
    /* Recreate the public key from scalar 1 without reaching through opaque
     * API. */
    if (verify_key) {
    ...
    if (signature && r && s && ECDSA_SIG_set0(signature, r, s) == 1) {
        ...
        WF_CHECK(ECDSA_do_verify(digest, sizeof(digest), signature,
                                 verify_key) == 1);
    } else {
        WF_CHECK(0);
    }
...
static void test_client_assertion(void) {
    ...
    sig_bytes =
        base64url_decode(second_dot + 1, strlen(second_dot + 1), &sig_len);
    header = header_bytes
                 ? cJSON_ParseWithLength((char *)header_bytes, header_len)
                 : NULL;
    payload = payload_bytes
                  ? cJSON_ParseWithLength((char *)payload_bytes, payload_len)
                  : NULL;
    ...
    WF_CHECK(cJSON_IsString(field) &&
             strcmp(field->valuestring, "auth-key-1") == 0);
    field = payload ? cJSON_GetObjectItemCaseSensitive(payload, "iss") : NULL;
    WF_CHECK(cJSON_IsString(field) &&
             strcmp(field->valuestring, options.client_id) == 0);
    ...
    WF_CHECK(cJSON_IsString(field) &&
             strcmp(field->valuestring, options.authorization_server_issuer) ==
                 0);
    field = payload ? cJSON_GetObjectItemCaseSensitive(payload, "jti") : NULL;
    WF_CHECK(cJSON_IsString(field) &&
             strcmp(field->valuestring, "assertion-jti") == 0);
