...
static void test_did_valid_extra(void) {
    ...
    WF_CHECK(wf_syntax_did_is_valid(
        "did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnSKS1Hoxg"));
}

static void test_did_invalid_extra(void) {
    WF_CHECK(
        !wf_syntax_did_is_valid("did:PLC:abc")); /* method must be lowercase */
    WF_CHECK(!wf_syntax_did_is_valid(
        "did:w3c:abc")); /* method must be letters only */
    WF_CHECK(!wf_syntax_did_is_valid("did:method:abc def"));
    WF_CHECK(!wf_syntax_did_is_valid("did:method:abc@")); /* @ not allowed */
    WF_CHECK(
        !wf_syntax_did_is_valid("did:method:abc/def")); /* '/' not allowed */
    WF_CHECK(!wf_syntax_did_is_valid(
        "did:method:abc%2")); /* incomplete pct-encoding */
...
static void test_handle_valid_extra(void) {
    ...
    WF_CHECK(
        wf_syntax_handle_is_valid("123.example.com")); /* digit-start label */
...
static void test_nsid_invalid_extra(void) {
    ...
    WF_CHECK(
        !wf_syntax_nsid_is_valid("com.example.foo-bar")); /* trailing dash... */
}
...
static void test_aturi_invalid_extra(void) {
    wf_syntax_aturi parsed = {0};
    WF_CHECK(!wf_syntax_aturi_parse(
        "at://did:plc:abc/com.example.post/extra/too", &parsed));
    wf_syntax_aturi_free(&parsed);
    WF_CHECK(!wf_syntax_aturi_parse("at://alice.bsky.social/bad_nsid/self",
                                    &parsed));
    wf_syntax_aturi_free(&parsed);
    WF_CHECK(!wf_syntax_aturi_parse("at://did:123:abc/com.example.post/self",
                                    &parsed));
    wf_syntax_aturi_free(&parsed);
...
static void test_datetime_valid_extra(void) {
    WF_CHECK(
        wf_syntax_datetime_is_valid("2024-02-29T00:00:00Z")); /* leap day */
    WF_CHECK(
        wf_syntax_datetime_is_valid("2024-12-31T23:59:60Z")); /* leap second */
...
static void test_datetime_invalid_extra(void) {
    WF_CHECK(
        !wf_syntax_datetime_is_valid("2023-02-29T00:00:00Z")); /* not leap */
    WF_CHECK(
        !wf_syntax_datetime_is_valid("2024-01-15T25:00:00Z")); /* hour 25 */
    WF_CHECK(!wf_syntax_datetime_is_valid("2024-01-15T12:30:00+24:00"));
    WF_CHECK(
        !wf_syntax_datetime_is_valid("24-01-15T12:30:00Z")); /* 2-digit year */
    WF_CHECK(!wf_syntax_datetime_is_valid(
        "2024-1-15T12:30:00Z")); /* 1-digit month */
...
int main(void) {
    test_did_valid_extra();
    test_did_invalid_extra();
    test_handle_valid_extra();
    test_handle_invalid_extra();
    test_at_identifier_valid_extra();
    test_at_identifier_invalid_extra();
    test_nsid_valid_extra();
    test_nsid_invalid_extra();
    test_record_key_valid_extra();
    test_record_key_invalid_extra();
    test_tid_valid_extra();
    test_tid_invalid_extra();
    test_aturi_valid_extra();
    test_aturi_invalid_extra();
    test_datetime_valid_extra();
    test_datetime_invalid_extra();
    test_language_valid_extra();
    test_language_invalid_extra();
    WF_TEST_SUMMARY();
}
