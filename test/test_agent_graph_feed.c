...
int main(void) {
    ...
    /* ── unmuteActorList (muteActorList is in moderation_actions.h) ── */
    {
        WF_CHECK(wf_agent_unmute_actor_list(
                     NULL, "at://did:plc:abc/app.bsky.graph.list/abc") ==
                 WF_ERR_INVALID_ARG);
        ...
        /* not logged in -> rejected before any network I/O */
        WF_CHECK(wf_agent_unmute_actor_list(
                     agent, "at://did:plc:abc/app.bsky.graph.list/abc") ==
                 WF_ERR_INVALID_ARG);
    ...
    /* ── getFeedSkeleton ──────────────────────────────────────────── */
    {
        WF_CHECK(wf_agent_get_feed_skeleton(
                     NULL, "at://did:plc:abc/app.bsky.feed.generator/abc", 0,
        ...
        WF_CHECK(wf_agent_get_feed_skeleton(agent, "", 0, NULL, &res) ==
                 WF_ERR_PARSE);
        WF_CHECK(wf_agent_get_feed_skeleton(agent, "did:plc:abc", 0, NULL,
                                            &res) == WF_ERR_PARSE);
    ...
    {
        wf_lex_app_bsky_graph_mute_actor_main_input m = {.actor =
                                                             "did:plc:abc"};
        char *m_json = NULL;
        WF_CHECK(wf_lex_app_bsky_graph_mute_actor_main_input_encode_json(
                     &m, &m_json) == WF_OK);
        ...
        char *ml_json = NULL;
        WF_CHECK(wf_lex_app_bsky_graph_mute_actor_list_main_input_encode_json(
                     &ml, &ml_json) == WF_OK);
