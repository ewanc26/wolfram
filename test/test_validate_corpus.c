...
int main(void) {
    ...
    if (!corpus_available(dir)) {
        printf("SKIP: atproto lexicon corpus not available (set "
               "WF_ATPROTO_LEXICONS)\n");
        WF_TEST_SUMMARY();
    }
    ...
    assert_invalid(r, "app.bsky.feed.post", "{\"text\":\"hello world\"}");
    ...
    assert_valid(r, "app.bsky.graph.listitem",
                 ...
                 "\"list\":\"at://did:plc:z72i7hdynmk6r22z27h6tvur/"
                 "app.bsky.graph.list/3jkl0pp8sic\","
    ...
    /* Value validation against a nested definition
    ...
    {
        const char *json = "{\"$type\":\"app.bsky.richtext.facet#byteSlice\","
                           "\"byteStart\":0,\"byteEnd\":5}";
        wf_validate_result res = wf_validate_value(
            r, "app.bsky.richtext.facet", "byteSlice", json, strlen(json));
        WF_CHECK(res.success == 1 && res.errors == NULL);
        wf_validate_result_free(&res);
    }
