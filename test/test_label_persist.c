...
int main(void) {
    ...
    WF_CHECK(acc_labels[0].cts &&
             strcmp(acc_labels[0].cts, "2020-01-01T00:00:00Z") == 0);
    ...
    WF_CHECK(wf_store_save_label(store, DID, NULL, "porn", LABELER,
                                 "2021-02-02T00:00:00Z", 0, 0, 0,
                                 NULL) == WF_OK);
    wf_mod_labels_free(acc_labels, acc_n);
    WF_CHECK(wf_store_load_labels(store, DID, &acc_labels, &acc_n) == WF_OK);
    WF_CHECK(acc_n == 1);
    WF_CHECK(acc_labels[0].cts &&
             strcmp(acc_labels[0].cts, "2021-02-02T00:00:00Z") == 0);
    ...
    /* Post label (porn, content target) on a different actor -> blur in content
     * view. */
    ...
    WF_CHECK(agent != NULL);
    /* The agent must know its own DID so load_labels_from_store can scope to
     * it. */
