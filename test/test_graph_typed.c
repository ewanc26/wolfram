...
int main(void) {
    ...
    WF_CHECK(wf_agent_parse_actors("{\"cursor\":\"x\"}", 13, &list) ==
             WF_ERR_PARSE);

    /* Wrapper arg validation. */
    wf_agent_actor_list out = {0};
    WF_CHECK(wf_agent_get_follows_typed(NULL, "did:plc:x", 10, NULL, &out) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_follows_typed(agent, NULL, 10, NULL, &out) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_follows_typed(agent, "did:plc:x", 10, NULL, NULL) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_followers_typed(NULL, "did:plc:x", 10, NULL, &out) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_search_actors_typed(agent, NULL, 10, NULL, &out) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_reposted_by_typed(agent, NULL, 10, NULL, &out) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_reposted_by_typed(agent, "at://x", 10, NULL, NULL) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_likes_typed(agent, NULL, 10, NULL, NULL) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_likes_typed(agent, "at://x", 10, NULL, NULL) ==
             WF_ERR_INVALID_ARG);

    const char *two_actors[] = {"did:plc:a", "did:plc:b"};
    wf_agent_actor_list plist = {0};
    WF_CHECK(wf_agent_get_profiles_typed(agent, NULL, 2, 10, NULL, &plist) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_profiles_typed(agent, two_actors, 0, 10, NULL,
                                         &plist) == WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_get_profiles_typed(agent, two_actors, 2, 10, NULL,
    ...
    if (json) {
        ...
        if (list.actor_count == 2) {
            WF_CHECK(list.actors[0].did &&
                     strcmp(list.actors[0].did, "did:plc:follower1") == 0);
            WF_CHECK(list.actors[0].handle &&
                     strcmp(list.actors[0].handle, "carol.example.com") == 0);
            WF_CHECK(list.actors[0].display_name &&
                     strcmp(list.actors[0].display_name, "Carol") == 0);
            WF_CHECK(list.actors[0].avatar &&
                     strstr(list.actors[0].avatar, "carol.jpg") != NULL);
            WF_CHECK(list.actors[1].did &&
                     strcmp(list.actors[1].did, "did:plc:follower2") == 0);
    ...
    /* likes list parse */
    {
        ...
        if (ljson) {
            ...
            WF_CHECK(likes.cursor &&
                     strcmp(likes.cursor, "likes-cursor-7") == 0);
            if (likes.like_count == 2) {
                WF_CHECK(likes.likes[0].actor.did &&
                         strcmp(likes.likes[0].actor.did, "did:plc:liker1") ==
                             0);
                WF_CHECK(likes.likes[0].created_at &&
                         strcmp(likes.likes[0].created_at,
                                "2026-07-02T11:00:00Z") == 0);
                WF_CHECK(likes.likes[0].indexed_at &&
                         strcmp(likes.likes[0].indexed_at,
                                "2026-07-02T11:00:01Z") == 0);
                WF_CHECK(likes.likes[1].actor.handle &&
                         strcmp(likes.likes[1].actor.handle,
                                "frank.example.com") == 0);
            }
    ...
    /* repostedBy list parse (profileView list under "repostedBy") */
    {
        ...
        if (rjson) {
            ...
            WF_CHECK(rb.actors[0].did &&
                     strcmp(rb.actors[0].did, "did:plc:reposter1") == 0);
    ...
    /* profiles list parse (profileView list under "profiles") */
    {
        ...
        if (pjson) {
            ...
            WF_CHECK(wf_agent_parse_profile_views(pjson, plen, "profiles",
                                                  &pv) == WF_OK);
            WF_CHECK(pv.actor_count == 2);
            WF_CHECK(pv.actors[0].avatar &&
                     strstr(pv.actors[0].avatar, "ivan.jpg") != NULL);
        ...
        if (rjson) {
            ...
            if (rl.rel_count == 2) {
                ...
                WF_CHECK(rl.rels[1].blocking_by_list &&
                         strstr(rl.rels[1].blocking_by_list, "listblock/lb1") !=
                             NULL);
            }
    ...
    {
        wf_agent_relationship_list rl = {0};
        WF_CHECK(wf_agent_parse_relationships(NULL, 0, &rl) ==
                 WF_ERR_INVALID_ARG);
        WF_CHECK(wf_agent_parse_relationships("{}", 2, NULL) ==
                 WF_ERR_INVALID_ARG);
        WF_CHECK(wf_agent_parse_relationships("not json", 8, &rl) ==
                 WF_ERR_PARSE);
        WF_CHECK(wf_agent_parse_relationships("{\"cursor\":\"x\"}", 13, &rl) ==
                 WF_ERR_PARSE);
    }
    ...
    /* muteThread / unmuteThread typed wrappers (offline arg validation). */
    {
        ...
        WF_CHECK(wf_agent_get_relationships_typed(agent, NULL, others, 1,
                                                  &rl) == WF_ERR_INVALID_ARG);
        WF_CHECK(wf_agent_get_relationships_typed(agent, "did:plc:me", others,
                                                  1,
                                                  NULL) == WF_ERR_INVALID_ARG);

        WF_CHECK(wf_agent_mute_thread_typed(NULL, "at://x") ==
                 WF_ERR_INVALID_ARG);
        ...
        WF_CHECK(wf_agent_mute_thread_typed(agent, "not-an-uri") ==
                 WF_ERR_INVALID_ARG);
        WF_CHECK(wf_agent_unmute_thread_typed(NULL, "at://x") ==
                 WF_ERR_INVALID_ARG);
