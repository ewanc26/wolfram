...
static void test_verify_record_commit_null_args(void) {
    ...
    WF_CHECK(wf_verify_record_commit(NULL, &dummy, 1, &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_verify_record_commit("", &dummy, 1, &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_verify_record_commit("zk", NULL, 0, &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_verify_record_commit("zk", &dummy, 0, &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_verify_record_commit("zk", &dummy, 1, NULL) ==
             WF_ERR_INVALID_ARG);
}
...
static void test_verify_record_commit_good_and_tampered(void) {
    ...
    if (wf_signing_key_generate(WF_KEY_TYPE_SECP256K1, &key2) != WF_OK) return;
...
static void test_agent_verify_record_null_args(void) {
    ...
    WF_CHECK(wf_agent_verify_record(NULL, "did", "coll", "rkey", &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_verify_record(agent, NULL, "coll", "rkey", &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_verify_record(agent, "", "coll", "rkey", &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_verify_record(agent, "did", NULL, "rkey", &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_verify_record(agent, "did", "", "rkey", &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_verify_record(agent, "did", "coll", NULL, &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_verify_record(agent, "did", "coll", "", &valid) ==
             WF_ERR_INVALID_ARG);
    WF_CHECK(wf_agent_verify_record(agent, "did", "coll", "rkey", NULL) ==
             WF_ERR_INVALID_ARG);
