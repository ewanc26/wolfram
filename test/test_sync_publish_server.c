...
static int event_equal(const wf_subscribe_event *a,
    ...
    if (a->type == WF_SUBSCRIBE_EVENT_COMMIT) {
        ...
        if (x->has_prev_data && !cid_equal(&x->prev_data, &y->prev_data))
            return 0;
        return 1;
    }
...
static int run_raw_client_e2e(struct test_ctx *ctx) {
    ...
    for (int i = 0; i < PUBLISHED_COUNT; i++) {
        ...
        if (!event_equal(&ctx->published[i], &dec)) {
            ...
            printf(
                "PASS: published #%s (seq %lld) round-trips through server\n",
...
static void on_event(const wf_subscribe_event *event, void *userdata) {
    ...
    if (idx < 0) {
        ctx->unexpected++;
        fprintf(stderr,
                "WARN: client received event with unexpected seq %lld\n",
                (long long)event->seq);
        ...
        fprintf(stderr,
                "FAIL: decoded event seq %lld did not match published\n",
                (long long)event->seq);
    }
...
static void *client_thread(void *arg) {
    ...
    char service[64];
    snprintf(service, sizeof(service), "ws://127.0.0.1:%u",
             (unsigned)ctx->port);
...
static int run_test(void) {
    ...
    if (!wf_websocket_supported()) {
        ...
        printf("NOTE: linked libcurl lacks WebSocket (ws) support; driving the "
               ...
               "sync_subscribe client's decoder) instead of "
               "wf_subscribe_start.\n");
