...
int main(void) {
    ...
    if (tmp_client) {
        WF_CHECK(wf_xrpc_upload_blob(NULL, "com.atproto.repo.uploadBlob", blob,
        ...
        WF_CHECK(wf_xrpc_upload_blob(tmp_client, NULL, blob, sizeof(blob),
                                     ...
                                     &res) == WF_ERR_INVALID_ARG);
        WF_CHECK(wf_xrpc_upload_blob(tmp_client, "com.atproto.repo.uploadBlob",
                                     NULL, sizeof(blob),
                                     "application/octet-stream",
                                     &res) == WF_ERR_INVALID_ARG);
        WF_CHECK(wf_xrpc_upload_blob(tmp_client, "com.atproto.repo.uploadBlob",
    ...
    if (client) {
        wf_status status =
            wf_xrpc_upload_blob(client, "com.atproto.repo.uploadBlob", blob,
                                sizeof(blob), "application/octet-stream", &res);
        WF_CHECK(status == WF_OK || status == WF_ERR_NETWORK ||
                 status == WF_ERR_HTTP);
    ...
    WF_CHECK(wf_auth_client_upload_blob(
                 NULL, "com.atproto.repo.uploadBlob", blob, sizeof(blob),
                 "application/octet-stream", &res) == WF_ERR_INVALID_ARG);
