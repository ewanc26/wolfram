...
int main(void) {
    static const wf_record_schema string_schema = {WF_RECORD_STRING, NULL, 0,
                                                   NULL};
    static const wf_record_schema integer_schema = {WF_RECORD_INTEGER, NULL, 0,
                                                    NULL};
    static const wf_record_schema boolean_schema = {WF_RECORD_BOOLEAN, NULL, 0,
                                                    NULL};
    static const wf_record_schema langs_schema = {WF_RECORD_ARRAY, NULL, 0,
                                                  &string_schema};
    static const wf_record_property props[] = {
        {"text", &string_schema, 1},     {"createdAt", &string_schema, 1},
        {"langs", &langs_schema, 0},     {"replyCount", &integer_schema, 0},
    ...
    WF_CHECK(langs && langs->type == WF_CBOR_ARRAY &&
             langs->children.count == 2);
    const wf_cbor_item *negative = get(record, "replyCount");
    WF_CHECK(negative && negative->type == WF_CBOR_NEGATIVE &&
             negative->neginteger == 1);
    wf_cbor_free(record);
    free(cbor);

    WF_CHECK(wf_record_encode_json("app.bsky.feed.post", &post,
                                   "{\"text\":\"missing timestamp\"}", &cbor,
                                   &len) == WF_ERR_INVALID_ARG);
    WF_CHECK(wf_record_encode_json("app.bsky.feed.post", &post,
                                   "{\"text\":1,\"createdAt\":\"now\"}", &cbor,
                                   &len) == WF_ERR_INVALID_ARG);
    WF_CHECK(wf_record_encode_json(
                 "app.bsky.feed.post", &post,
    ...
    WF_CHECK(wf_record_encode_json(
                 "app.bsky.feed.post", &post,
                 "{\"text\":\"x\",\"text\":\"y\",\"createdAt\":\"now\"}", &cbor,
                 &len) == WF_ERR_INVALID_ARG);
