...
typedef struct wf_server_app_password {
    char *name;
    char
        *password; /* NULL for entries from listAppPasswords (never returned) */
    char *created_at;
 ...
 */
wf_status
wf_server_revoke_app_password(wf_xrpc_client *client,
...
typedef struct wf_server_create_invite_codes_input {
    ...
    const char *const
        *for_accounts; /* optional; restrict each code to accounts */
 ...
 */
wf_status
wf_server_revoke_invite_codes(wf_xrpc_client *client,
                              const wf_server_revoke_invite_codes_input *input);

/** Activate the authenticated account. Calls POST
...
/** Deactivate the authenticated account. Calls POST
...
/** Request a fresh email-confirmation message. Calls POST
...
typedef struct wf_server_update_email_input {
    ...
    int has_email_auth_factor; /* set non-zero when email_auth_factor is
                                  meaningful */
...
typedef struct wf_service_auth_request {
    ...
    const char
        *lxm; /* optional; lexicon (XRPC) NSID to bind to; NULL to omit */
