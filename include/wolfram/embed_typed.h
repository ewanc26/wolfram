 ...
 * `associatedRecords` shapes are open/unbounded and kept as owned detached
 * cJSON subtrees. Free with wf_embed_external_view_free (safe on a reset
 * struct). */
typedef struct wf_embed_external_view {
    ...
    cJSON *associated_refs;    /* owned detached "associatedRefs" array; NULL
                                  absent */
    cJSON *associated_records; /* owned detached "associatedRecords" array; NULL
                                  absent */
 ...
 * parses the body into `out`. On success `out` is owned by the caller (free
 * with wf_embed_external_view_free); on error it is left reset. Returns
 ...
 * cJSON subtrees, mirroring the feed-typed convention that keeps
 * `record`/`embed` as detached cJSON. Every owned string is heap-allocated and
 * every struct has a matching `_free` (safe on a reset/zeroed struct; `memset`
 * after free).
 */
 ...
 * several shapes; we discriminate on the inner object's keys / `$type` and
 * parse the common viewRecord form in full, detaching the others into `extra`.
 */
typedef enum {
