...
wf_status wf_agent_chat_add_reaction(wf_agent *agent, const char *convo_id,
                                     const char *message_id, const char *value);
...
wf_status wf_agent_chat_disable_join_link(wf_agent *agent,
                                          const char *convo_id);
wf_status wf_agent_chat_enable_join_link(wf_agent *agent, const char *convo_id);
...
/* Subscriptions are never code-generated by wf_lexgen (it only emits a
 * `_main_call` for "query"/"procedure" kinds; see tools/wf_lexgen.cpp), so
 * there is no generated call to route this through — matching
 * `com.atproto.sync.subscribeRepos` (sync_subscribe.c) and
 * `com.atproto.label.subscribeLabels` (label.h's WF_LABEL_SUBSCRIBE_NSID),
 * which hand-build their WS URLs the same way. This alias to the generated
 * NSID *constant* (the one piece of generated marshalling that does exist
 * for a subscription — the `WF_LEX_..._NSID`/`_KIND` `#define`s emitted for
 * every def) at least ties the literal to the lexicon instead of duplicating
 * it, so a corpus change can't silently desync the two. */
#define WF_CHAT_MOD_EVENTS_NSID                                                \
    WF_LEX_CHAT_BSKY_MODERATION_SUBSCRIBE_MOD_EVENTS_NSID
...
typedef struct wf_chat_mod_events_options {
    ...
    /* Optional bearer access token for this "private endpoint"; sent as
     * `Authorization: Bearer <access_token>` on every (re)connect via
     * wf_websocket_connect_with_headers. NULL omits the header entirely
     * (unauthenticated connection attempt, matching the previous behavior). */
 ...
 *
 * This is a "private endpoint" that requires moderator authentication: the
 * agent's current session access token is read via wf_agent_get_session_data
 * and sent as an `Authorization: Bearer <token>` header on every (re)connect
 * (wf_websocket_connect_with_headers). A session with no access token yet
 * (never logged in) connects without the header, matching a public
 * subscription — the server is the one place that can reject an
 * unauthenticated attempt to this endpoint. */
